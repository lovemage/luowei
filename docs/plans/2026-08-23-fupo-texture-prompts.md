...
| 類別 | 數量 | 素材 | 55% 對比 |
| --- | --- | --- | --- |
| 創會理念．編號條目 | 3 | `belief-02` | 5.00 |
| 理念收尾金句 | 1 | `belief-closing`（`cover`） | 5.19 |
| 系統．編號條目 | 4 | `system-01` | 5.17 |
| 系統．規模數字 | 4 | `stat-04` | — |
| 小標籤（接觸點＋產業鏈項目） | 7 + 全部 | `touch-a`（`cover`） | — |
| 產業鏈．導覽格 | 15 | `chain-label-12` | — |
| 產業鏈．細節卡 | 15 | `chain-bg-01` | — |
| 產業鏈．「舉個例」 | 15 | `system-03`（`cover`） | 5.06 |

接觸點標籤與產業鏈項目標籤是同一種視覺元件（細框小標籤），所以合併成同一張。

頁面實際引用的素材從 20 張降到 8 張（另加 `cta-idle`、`cta-active`、`modal-bg` 共 11 張）。
...
收斂成一類一張後，下列 12 張已從頁面移除引用，但檔案仍留著（想換配置時不必重產）：

```
belief-01  belief-03  system-04  stat-03  touch-b
